...
class genetic:
    ...
    def newGenes(self, genome):
        ...
        for idx in range(len(self.initial)):
            genome.append(
                (randint(1, 60), self.initial[idx][1], self.initial[idx][2]))
    ...
    def run(self):
        for idx in range(self.iterations):
            ...
            population = [idx for _, idx in sorted(
                zip(self.fitnessValues, self.population), reverse=True)]

            # order the fitness from high to low
            self.fitnessValues = sorted(self.fitnessValues, reverse=True)
            prob = [(element / sum(self.fitnessValues))
                    for element in self.fitnessValues]

            file.write("Avg. Fitness:{} sd {} \n".format(
                np.mean(self.fitnessValues), np.std(self.fitnessValues)))
            ...
            # copy the best chromosomes into the new population
            for pop_index in range(elite):
